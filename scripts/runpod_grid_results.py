...
for config, seed, runpod_id in JOB_IDS:
    ...
    print(
        f"  {config}/s{seed}: acc={metrics.get('accuracy', 0):.4f} sharpe={meta.get('sharpe_ratio', 'n/a')}"
    )
...
print(f"{'=' * 90}")
print(
    f"  {'Rank':<5} {'Config':<15} {'Seed':<6} {'Acc':>8} {'Sharpe':>8} {'PBO':>6} {'DSR':>8} {'Brier':>8} {'BestIter':>8}"
)
print(
...
for i, r in enumerate(results):
    ...
    print(
        f"  {i + 1:<5} {r['config']:<15} {r['seed']:<6} {acc:>8.4f} {sharpe:>8.4f} {pbo:>6.2f} {dsr:>8.4f} {brier:>8.4f} {best_iter!s:>8}"
    )
...
print(f"\n{'=' * 90}")
print("GRID SEARCH COMPLETE — 12/12 jobs succeeded")
print(f"{'=' * 90}")
