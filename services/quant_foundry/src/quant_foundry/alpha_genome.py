...
class AlphaGenomeLab:
    ...
    def _build_evidence(self, outcome: Any) -> Any:
        ...
        feature_set_version, pit_evidence, backend_eligible). The alpha
        genome lab path doesn't have a DB-backed metrics registry, so C7
        evidence is pulled from the outcome's optional ``c7_evidence``
        attribute (a dict) or defaulted from the dossier when available.
        """
        try:
        ...
        c7 = getattr(outcome, "c7_evidence", None) or {}
        selfcheck = c7.get("selfcheck")
        callback_receipt = c7.get("callback_receipt")
        artifact_uri = c7.get("artifact_uri")
        dossier_hash = c7.get("dossier_hash")
        if dossier_hash is None and dossier is not None:
            dossier_hash = dossier.content_hash
        feature_set_version = c7.get("feature_set_version")
        pit_evidence = c7.get("pit_evidence")
        backend_eligible = c7.get("backend_eligible", False)
