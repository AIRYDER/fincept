"""
quant_foundry.modules.sentiment.llm_xai — xAI (Grok) LLM sentiment provider.

Scores media items for sentiment using xAI's Grok chat completion API.
Uses ``httpx`` directly (no SDK dependency).  The API key is read from
``XAI_API_KEY``.

xAI's Grok is particularly good at social media text (it's integrated
with X/Twitter), making it a strong choice for the social-text arm of
the FinBERT + LLM hybrid.

This module is registered as ``sentiment:llm-xai:1.0.0``.
"""
...
class XAISentiment:
    """xAI (Grok) LLM sentiment provider.
    ...
    def _get_api_key(self) -> str:
        ...
        if not key:
            raise ValueError(
                "XAI_API_KEY is not set. Set it in the environment or RunPod container env."
            )
        return key

    def score(self, items: list[MediaItem]) -> list[SentimentResult]:
        """Score media items using xAI chat completions."""
        import httpx
        ...
        try:
            ...
            return [
                SentimentResult(item_id=item.item_id, provider="xai", score=0.0, confidence=0.0)
                for item in items
            ]
