...
class ComparisonReport:
    ...
    def _compute(self) -> None:
            ...
            self.summary_table.append(
                {
                    ...
                    "pbo": r.pbo,
                    "row_count": (r.dossier.metadata.get("n_rows", "?") if r.dossier else "?"),
                    "n_features": (r.dossier.metadata.get("n_features", "?") if r.dossier else "?"),
        ...
        for r in successful:
            source_key = (
                r.config.source.split(":")[1] if ":" in r.config.source else r.config.source
            )
        ...
        for r in successful:
            sentiment_key = (
                r.config.sentiment.split(":")[1]
                if ":" in r.config.sentiment
                else r.config.sentiment
            )
    ...
    def summary_text(self) -> str:
        ...
        if self.ranked_by_sharpe:
            ...
            for i, entry in enumerate(self.ranked_by_sharpe, 1):
                dsr = (
                    f"{entry['deflated_sharpe']:.4f}"
                    if entry["deflated_sharpe"] is not None
                    else "N/A"
                )
        ...
        if self.best_by_source:
            ...
            for source, info in self.best_by_source.items():
                dsr = (
                    f"{info['deflated_sharpe']:.4f}"
                    if info["deflated_sharpe"] is not None
                    else "N/A"
                )
