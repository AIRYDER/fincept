...
Source of truth for prior state: `docs/runpod-fix-plan/RECEIPT_INDEX.md` (pass #5, **uncommitted** — see D5)
...
**No code has landed since v4 was written** — HEAD is still `677c77ed`,
no v4 task has been started. Every v4 task is still OPEN. v5 adds one new
task (D5) and corrects B1's dirty-worktree list.
...
| D4 commit ci-triage receipts | **OPEN** → D4 | unchanged; blocked by B1 |
| — | **NEW** → D5 | commit pass #5 `RECEIPT_INDEX.md` + v4/v5 task queues; blocked by B1 |

### Stale instruction in committed RECEIPT_INDEX.md (note, resolved by D5)
...
- `docs/runpod-fix-plan/RECEIPT_INDEX.md` — **modified** (pass #5
  consolidation: 3/3 → 6/6 canary, second independent run finding #8, pass #5
  CI status, stale-instruction correction). **Durable evidence — candidate
  for committing (see D5).** v4 missed this item; v5 adds it.
...
- `docs/runpod-fix-plan/09-swarm-task-queue-v3.md` — untracked, superseded
  by v4/v5. Drop or keep for history.
- `docs/runpod-fix-plan/10-swarm-task-queue-v4.md` — untracked, superseded
  by v5. Keep for history (commit via D5).
- `docs/runpod-fix-plan/11-swarm-task-queue-v5.md` — untracked (this file).
  Current source of truth (commit via D5).
...
- Only after operator approval: `SESSION_HANDOFF.md`, `handoffs/`,
  `kimiSuggestionFix.md`, `reports/ci-triage/`, the task-queue docs,
  `docs/runpod-fix-plan/RECEIPT_INDEX.md`.
